...
import { useStakingContract, useStakingTokenContract } from 'hooks/useCreateContract'
...
const Index = () => {
    ...
    async (customer, token, dataModal, amount) => {
      ...
      try {
        contractBep20.methods
          ...
          .on('error', () => {
            // setMessError(get(error, "message"))
            setBtnLoading(false)
  ...
  //   if (!contract) {
  //     return toastError('Contract is null.!')
  //   }
  ...
  //         .on('receipt', (receipt) => {
  //           // eslint-disable-next-line no-console
  //           console.log('E0009', receipt)
  //           toastSuccess('Harvest Success')
  //         })
  //     )
  //   } catch (e) {
  //     // eslint-disable-next-line no-console
  //     return console.log('E0009', e)
  //   }
  //   // eslint-disable-next-line react-hooks/exhaustive-deps
  // }, [account, contract])
  ...
  return (
    <>
      <ModalCustom
        ...
        visible={MODAL_TYPES.withdraw === get(modalOverview, 'type')}
        title={(() => {
          const swapFrom = get(modalOverview, 'title', '')
          return <>{`Withdraw ${swapFrom}`}</>
        })(modalOverview)}
        width={400}
